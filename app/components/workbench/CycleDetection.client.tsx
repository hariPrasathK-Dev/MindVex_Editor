...
export function CycleDetection() {
  ...
  const analyzeCycles = async () => {
    ...
    try {
        ...
        try {
          ...
          for (const entry of entries) {
            ...
            if (entry.isFile()) {
              ...
              const allowedExtensions = ['.py', '.java', '.js', '.jsx', '.ts', '.tsx', '.c', '.cpp', '.cs', '.rb', '.php', '.go', '.rs', '.kt', '.swift', '.scala', '.dart', '.html', '.css', '.scss', '.sql', '.json', '.yaml', '.yml', '.xml', '.md'];
      ...
      console.log('Code file extensions:', codeFiles.map(f => pathUtils.extname(f.path)).filter(ext => ext));
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-2">
              Detect cyclic dependencies in your codebase architecture
            </p>
          </div>
              ...
              <div className="bg-gray-700/50 p-4 rounded-lg">
                ...
                {detectionResults ? (
                  <div className="flex items-center">
                    <div className={`w-3 h-3 rounded-full mr-2 ${detectionResults.hasCycles ? 'bg-red-500' : 'bg-green-500'}`}></div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {detectionResults ? detectionResults.cycles.length : 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Total Nodes:</span>
                    <span className="font-medium">
                      {graph ? graph.nodes.length : 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-300">Total Dependencies:</span>
                    <span className="font-medium">
                      {graph ? graph.edges.length : 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  {detectionResults.cycles.map((cycle, index) => (
                    <div key={index} className="bg-gray-700/30 p-4 rounded-lg">
                              ...
                              <div key={nodeIndex} className="flex items-center">
                                {node ? (
                                  ...
                                  <span className="bg-gray-600 px-2 py-1 rounded">
                                    <div className="text-xs text-gray-300">{nodeId.includes('#') ? pathUtils.basename(nodeId.split('#')[0]) : 'unknown'}</div>
                                    <div>{displayName}</div>
                                  </span>
                                )}
                      ...
                      <div className="mt-2 pt-2 border-t border-gray-600 text-xs text-gray-400">
                        <div>Files involved: {[...new Set(cycle.map(nodeId => {
                          const node = graph?.nodes.find(n => n.id === nodeId);
                          return node ? pathUtils.basename(node.filePath) : nodeId.includes('#') ? pathUtils.basename(nodeId.split('#')[0]) : 'unknown';
                        }))].join(', ')}</div>
                      </div>
                    </div>
                  ))}
        ...
        <div className="bg-gray-800/50 backdrop-blur-lg rounded-xl p-6 border border-gray-700">
          ...
          <div className="prose prose-invert max-w-none text-gray-300">
            <p>
              Cycle Detection identifies circular dependencies in your codebase architecture. 
              Cyclic dependencies can lead to tight coupling, making the code harder to understand, 
              test, and maintain. This analysis helps identify these problematic patterns.
            </p>
