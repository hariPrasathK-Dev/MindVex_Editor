      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-2">
              Analyze the impact of changes to specific nodes in your codebase
            </p>
          </div>
                      ...
                      {impactResults
                        ...
                        .map((node, index) => (
                          <li key={index} className="text-sm text-gray-300 flex items-start">
                            ...
                            <span>
                              <span className="font-medium">{node.name}</span> ({node.type}) in {pathUtils.basename(node.filePath)}
                            </span>
                          </li>
                        ))}
                    </ul>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium text-blue-400">
                        {impactResults.filter(r => r.impact_level === 'direct').reduce((sum, r) => sum + r.impacted_nodes.length, 0)}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-400">Indirect Impacts:</span>
                      <span className="ml-2 font-medium text-orange-400">
                        {impactResults.filter(r => r.impact_level === 'indirect').reduce((sum, r) => sum + r.impacted_nodes.length, 0)}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-lg rounded-xl p-6 border border-gray-700">
          ...
          <div className="prose prose-invert max-w-none text-gray-300">
            <p>
              Change Impact Analysis identifies the potential effects of modifying a specific node in your codebase. 
              It analyzes both direct dependencies (nodes that directly depend on the changed node) and indirect impacts 
              (nodes that depend on the directly impacted nodes).
            </p>
            <ul className="list-disc pl-5 mt-2 space-y-1">
              <li><span className="text-blue-400">Direct Impact:</span> Nodes that directly depend on the changed node</li>
              <li><span className="text-orange-400">Indirect Impact:</span> Nodes that depend on directly impacted nodes</li>
            </ul>
          </div>
        </div>
      </div>
