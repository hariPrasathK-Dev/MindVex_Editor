...
export function QuickActions() {
  ...
  const handleActionClick = async (actionId: string) => {
    if (actionId === 'knowledge-graph') {
      ...
      alert('Real-Time Graph Update: This feature tracks file changes and updates the graph incrementally in real-time.');
    } else {
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-gray-400 mt-2">
                AST-based analysis of your codebase relationships
              </p>
            </div>
            <button
              onClick={buildGraph}
          ...
          {graph ? (
                      ...
                      {graph.nodes.map((node: any, index: number) => (
                        <div 
                          ...
                          className={`p-4 rounded-lg border ${
                            node.type === 'module' ? 'border-blue-500/30 bg-blue-900/20' :
                            node.type === 'class' ? 'border-purple-500/30 bg-purple-900/20' :
                            node.type === 'function' ? 'border-green-500/30 bg-green-900/20' :
                            'border-gray-500/30 bg-gray-700/20'
                          }`}
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`inline-block w-3 h-3 rounded-full ${
                              node.type === 'module' ? 'bg-blue-500' :
                              node.type === 'class' ? 'bg-purple-500' :
                              node.type === 'function' ? 'bg-green-500' :
                              'bg-gray-500'
                            }`}></span>
                          ...
                          <tr key={edge.id}>
                            ...
                            <td className="px-4 py-3 text-sm">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                edge.type === 'import' ? 'bg-blue-100 text-blue-800' :
                                edge.type === 'call' ? 'bg-purple-100 text-purple-800' :
                                edge.type === 'inheritance' ? 'bg-green-100 text-green-800' :
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-gray-400 max-w-md">
                Analyzing your codebase to construct a knowledge graph of modules, classes, functions, and their relationships.
              </p>
            </div>
          )}
        </div>
    ...
    <div className="h-full bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 overflow-y-auto">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-2">
            Select an action to analyze your codebase
          </p>
        </div>
          ...
          {quickActions.map((action) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`text-2xl ${action.enabled ? '' : 'grayscale'}`}>
                  {action.icon}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex justify-end">
                    {loading === action.id ? (
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-medium bg-${action.color}-500/20 text-${action.color}-400`}>
                        Available
                      </span>
                    ) : (
        ...
        <div className="mt-8 p-4 bg-gray-800/30 backdrop-blur-lg rounded-xl border border-gray-700">
          ...
          <p className="text-gray-400 text-sm">
            Click on any action to execute it. The Knowledge Graph Construction is currently available and will analyze your Java and Python files.
          </p>
        </div>
      </div>
    </div>
