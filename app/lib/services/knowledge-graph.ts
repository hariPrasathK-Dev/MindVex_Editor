  ...
  parse(code: string, filePath: string): { nodes: Node[], edges: Edge[] } {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const callMatch of callMatches) {
        ...
        if (!['print', 'len', 'str', 'int', 'float', 'list', 'dict', 'set', 'tuple', 'range', 'input', 'open'].includes(functionName)) {
  ...
  parse(code: string, filePath: string): { nodes: Node[], edges: Edge[] } {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const callMatch of callMatches) {
        ...
        if (!['System.out.println', 'println', 'main', 'toString', 'equals', 'hashCode', 'getClass', 'wait', 'notify', 'notifyAll', 'finalize'].includes(targetFunc)) {
          // Look for the target function in nodes
          const targetNode = nodes.find(node => 
            node.name === targetFunc && 
            node.type === 'function' &&
            node.filePath === filePath
          );
...
export class IncrementalGraphUpdater {
  ...
  private removeNodesForFiles(filePaths: string[]) {
    ...
    this.graphCache.nodes = this.graphCache.nodes.filter(node => 
      !filePaths.some(path => node.filePath.startsWith(path))
    );
    
    // Remove edges that connect to removed nodes
    const nodeIds = new Set(this.graphCache.nodes.map(n => n.id));
    this.graphCache.edges = this.graphCache.edges.filter(edge => 
